...
(function (angular, buildfire) {
  angular
    ...
    .controller('WidgetRedeemCtrl', ['$scope', 'ViewStack', 'VoteCache', 'LoyaltyAPI', '$timeout', '$rootScope', 'Buildfire', 'Context',
      function ($scope, ViewStack, VoteCache, LoyaltyAPI, $timeout, $rootScope, Buildfire, Context) {
        ...
        if (VoteCache.getReward()) {
          WidgetRedeem.reward = VoteCache.getReward();
        }

        /**
         * Initialize WidgetRedeem.application with current loyalty app details set in home controller
         */
        if (VoteCache.getApplication()) {
          WidgetRedeem.application = VoteCache.getApplication();
        }
